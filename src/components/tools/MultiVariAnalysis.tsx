...
export function MultiVariAnalysis({ toolId = "multi-vari", toolName = "Multi-Vari-analys", phase = 3 }: Props) {
  ...
  const groupVariance = (groups: string[], getGroup: (p: DataPoint) => string) => {
    ...
    const groupMeans = groups.map(g => {
      const gv = points.filter(p => getGroup(p) === g).map(p => p.value);
      return gv.length ? gv.reduce((a, b) => a + b, 0) / gv.length : 0;
    });
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-1">
          <Label className="text-xs">Inom enhet (position/mätpunkt)</Label>
          <Input value={form.withinUnit} onChange={e => setForm({ ...form, withinUnit: e.target.value })} placeholder="T.ex. Topp, Mitten, Botten" className="text-sm" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Mellan enheter (enhet/batch)</Label>
          <Input value={form.betweenUnit} onChange={e => setForm({ ...form, betweenUnit: e.target.value })} placeholder="T.ex. Enhet 1, Enhet 2" className="text-sm" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-1">
          <Label className="text-xs">Temporal (tidpunkt)</Label>
          <Input value={form.temporal} onChange={e => setForm({ ...form, temporal: e.target.value })} placeholder="T.ex. FM, EM" className="text-sm" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Mätvärde</Label>
          <Input type="number" value={form.value} onChange={e => setForm({ ...form, value: e.target.value })} placeholder="10.5" className="text-sm" />
        </div>
      </div>
        ...
        <div className="border rounded-lg overflow-x-auto max-h-40 overflow-y-auto">
          <table className="w-full text-xs">
            <thead className="bg-muted/50 sticky top-0">
              <tr>
                <th className="p-1.5 text-left">Inom</th>
                <th className="p-1.5 text-left">Mellan</th>
                <th className="p-1.5 text-left">Tid</th>
                <th className="p-1.5 text-right">Värde</th>
            ...
            <tbody>
              {points.map(p => (
                <tr key={p.id} className="border-t">
                  <td className="p-1.5">{p.withinUnit || "–"}</td>
                  <td className="p-1.5">{p.betweenUnit || "–"}</td>
                  <td className="p-1.5">{p.temporal || "–"}</td>
                  <td className="p-1.5 text-right font-mono">{p.value}</td>
            ...
            {[{ label: "Inom enhet", val: withinVar }, { label: "Mellan enheter", val: betweenVar }, { label: "Temporal", val: temporalVar }].map(v => (
              <div key={v.label} className="flex items-center gap-2">
                ...
                <div className="flex-1 h-3 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-primary rounded-full" style={{ width: `${(v.val / totalVar * 100).toFixed(0)}%` }} />
                </div>
